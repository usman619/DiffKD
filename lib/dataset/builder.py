...
def build_dataloader(args):
    ...
    if args.dataset == 'imagenet':
        ...
        args.data_path = 'data/cifar100' if args.data_path == '' else args.data_path
    ...
    if args.dataset == 'imagenet':
        ...
        train_transforms_l, train_transforms_r = transform.build_train_transforms_cifar10(
            args.cutout_length, args.image_mean, args.image_std)
        # Check if ImageFolder format (Kaggle) or binary format
        train_path = os.path.join(args.data_path, 'train')
        if os.path.isdir(train_path) and len(os.listdir(train_path)) > 0 and os.path.isdir(os.path.join(train_path, os.listdir(train_path)[0])):
            # ImageFolder format (Kaggle dataset)
            train_dataset = ImageFolder(train_path, transform=train_transforms_l)
        else:
            # Binary format (standard CIFAR100)
    ...
    if args.dataset == 'imagenet':
        ...
        if os.path.isdir(val_path) and len(os.listdir(val_path)) > 0 and os.path.isdir(os.path.join(val_path, os.listdir(val_path)[0])):
            # ImageFolder format (Kaggle dataset)
